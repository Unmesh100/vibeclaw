...
${serviceWorkerScript}
  ...
  const postToParent = (message) => {
    parent.postMessage({ ...message, runtimeId }, '*');
  };
  ...
  window.addEventListener('message', async (event) => {
    ...
    // If initialized, only accept messages for this runtime instance.
    if (runtimeId && incomingRuntimeId && incomingRuntimeId !== runtimeId) {
      return;
    }
    ...
    try {
      switch (type) {
        case 'init':
          runtimeId = incomingRuntimeId || runtimeId;
        ...
        case 'execute':
          if (!runtime) {
            postToParent({ type: 'error', id, error: 'Runtime not initialized' });
            return;
          }
          const execResult = runtime.execute(code, filename);
          postToParent({ type: 'result', id, result: execResult });
          break;
        ...
        case 'runFile':
          ...
          postToParent({ type: 'result', id, result: runResult });
          break;
