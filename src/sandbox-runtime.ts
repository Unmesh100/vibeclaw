...
export class SandboxRuntime implements IRuntime {
  ...
  private messageId = 0;
  private runtimeId = `sandbox-${Math.random().toString(36).slice(2)}-${Date.now().toString(36)}`;
  ...
  private setupMessageHandler(): void {
    this.messageHandler = (event: MessageEvent) => {
      // Only accept messages from our sandbox origin and iframe instance.
      if (event.origin !== this.sandboxOrigin) return;
      if (event.source !== this.iframe.contentWindow) return;

      const message = event.data as SandboxMessage;

      // Route only messages for this runtime (ready can be runtimeId-less for compatibility).
      if (message.type !== 'ready' && message.runtimeId && message.runtimeId !== this.runtimeId) {
        return;
      }
    ...
    return new Promise((resolve) => {
      const handler = (event: MessageEvent) => {
        ...
        if (message.type === 'ready') {
          if (message.runtimeId && message.runtimeId !== this.runtimeId) {
            return;
          }
  ...
  private sendAndWait(message: SandboxMessage): Promise<IExecuteResult> {
    return new Promise((resolve, reject) => {
      ...
      this.iframe.contentWindow?.postMessage(
        { ...message, runtimeId: this.runtimeId, id },
        this.sandboxOrigin
      );
   ...
   */
  clearCache(): void {
    const message: SandboxMessage = { type: 'clearCache', runtimeId: this.runtimeId };
    this.iframe.contentWindow?.postMessage(message, this.sandboxOrigin);
  }
